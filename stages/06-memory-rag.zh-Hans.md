...
> 💡 这 stage 用语密度高（**RAG / 向量数据库 / embedding / chunking / hybrid search / reranking⋯**）→ 不熟先翻 [`resources/glossary.md` §3](../resources/glossary.md#3-memory--retrieval--rag)。

> 📋 **本章组成**：〔Context Engineering 是什么（先定位）+ 概念对照表 ×2〕→ 学习目标 → 进入条件 → 必修阅读 → 单元指引 → Chunking → Memory 设计三种 pattern → 进阶 Memory（CoALA / Generative Agents / **2024-2026 纵览**）→ Reflexion 完整版 → 进阶 Reasoning（Path 1 prompt-based + **Path 2 o1/R1/R2/Opus 4.7/GPT-5.5 trained-in**）→ 进阶 RAG 技巧（GraphRAG / Contextual Retrieval / Hybrid Search / Query Trans / Self-improving / RAPTOR / **2024-2026 纵览含 MiA-RAG / A-RAG / MegaRAG**）→ 动手练习 → 常用工具推荐 → 精选 Projects → 自我检查  
...
| 3 | **Harness Engineering** | 把多个 LLM call 包成 production runtime | [Stage 7 §Harness Engineering](07-multi-agent-production.md#-harness-engineering--production-agent-runtime-的工程学--本-stage-核心概念) |

**Context Engineering 的 3 个 problem domain**（本 stage 主轴是前两个）：

1. **Memory 管理** — short-term / long-term / episodic / semantic memory 怎么分层、怎么存、怎么忘
2. **Retrieval** — 怎么从外部知识库捞相关片段（RAG / vector search / GraphRAG / hybrid search）
...
| **Embedding** | 把文字转成 N 维**向量**、让相似度可计算（数据转换） | `sentence-transformers` 跑出 768 维向量 / OpenAI ada-002 |
...
没到的话 → 回 [Stage 3](03-tool-use-and-hello-agent.md) 或 [Stage 0 §环境设定](00-foundations.md#环境设定)。
...
这会带到更进阶的 RAG 技术，例如 GraphRAG。有兴趣的同学可以思考，为何这种情境要设计这样的 RAG 解决方案，不用实作每种 RAG 技术或细节。
...
- 想想看，如果今天要 RAG 的数据有含图片的 PDF、会议字幕档，要如何切割比较好？

## 🧠 Memory 设计三种 pattern（什么时候用什么）⭐ Track B 必看

**不是所有 agent 都需要 RAG。Memory 架构选错会花十倍 token 达同样效果。**
...
### CoALA framework — agent memory 的 4 层 taxonomy
...
| **Procedural memory** | 怎么做事的程序 / skill | tool definitions、[Skills（Stage 5.3）](05-claude-code-ecosystem.md#53--skills--可重复使用的-prompt--instruction-包) |

→ **为什么有用**：上面 3 种 pattern（buffer / summary / vector）都只在处理 working + episodic。Production agent 4 层都要设计——CoALA 是检查表，看看你的 agent 哪一层缺了。

### Generative Agents — 三分数打分（经典案例）
...
| **MemoryLLM** | self-updatable memory parameters 内建在 model（在权重而非 context）| [Wang et al. 2024](https://arxiv.org/abs/2402.04624) |
| **mem0**（已在上面列） | production memory layer、auto fact extraction + forgetting | [mem0ai/mem0](https://github.com/mem0ai/mem0) 2024 |
| **Memory in the Age of AI Agents**（survey）| 系统 survey、3 维 taxonomy（temporal scope / substrate / control policy）+ benchmark 汇整 | [Hu et al. arXiv:2512.13564](https://arxiv.org/abs/2512.13564) ⭐ **2025-12** |
...
**为什么这个版本要 memory**：Reflexion paper 的 verbal reinforcement learning 是「agent 跨 trial 累积教训」——agent 尝试 task → 失败 → 反思「为什么失败」存起来 → 下次遇到类似 task 时把过去反思 retrieve 进 prompt，避免重蹈覆辙。这需要 **persistent episodic memory**，跟本 stage 上面讲的 3 种 memory pattern 直接接上。

**典型架构**：
```
Stage 3 §反思（基本版）                Stage 6 本节（完整版）
─────────────────────                  ─────────────────────
...
**Reference 实作**：
- [**noahshinn/reflexion**](https://github.com/noahshinn/reflexion) — paper 第一作者的 reference 实作（含 episodic memory 完整流程）
...
| **DeepSeek-R2** | DeepSeek 2026-03（前身：R1 2025-01）| 开源 RL+CoT、**MIT license**、AIME 2025 **79.7%**（R1 为 39.4%）、GPQA Diamond 72.0% | [DeepSeek guide 2026](https://deepseek.ai/blog/deepseek-guide-2026)、[R1 paper（方法 baseline）](https://arxiv.org/abs/2501.12948) |
| **DeepSeek-V4 / V4-Pro / V4-Flash** | DeepSeek 2026-04 preview | 开源、agent-focused 训练、推理 + 工具使用 + 知识处理整合 | [HF DeepSeek-V4-Pro](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro)、[CNBC report](https://www.cnbc.com/2026/04/24/deepseek-v4-llm-preview-open-source-ai-competition-china.html) |
...
| 预算 / latency 允许、要最强 reasoning | Path 2 —— **GPT-5.5 / Opus 4.7 / Gemini 3.1 Pro / R2** 任挑一个 |
| 想自己 fine-tune reasoning model | Path 2 —— 读 R1 + R2 paper、从 R1-Distill 系列起步 |
...
> - **2026 开源已追上闭源**——R2 的 AIME 2025 达 79.7%、跟 GPT-5.5 / Gemini 3.1 Pro 同档次、且 MIT license
...
**先跑完上面练习 1-5 拿到基准版本、再回来看这里**——不然你会在没有基准的情况下调参数，永远不知道是哪个改动带来提升。
...
**何时不用**：
- 文件之间没有 entity-relation 连结（纯 FAQ、产品手册各自独立）
- 知识库小（< 1k chunk）——vanilla RAG 已经够
- 预算紧——建 KG 的 token 成本可能是普通 RAG 的 10-50 倍

**代表 framework**：
- [**Microsoft GraphRAG**](https://github.com/microsoft/graphrag) ⭐ — 原版 reference 实作、Apache-2.0、含 community detection
- [**HKUDS/LightRAG**](https://github.com/HKUDS/LightRAG) — 轻量版、EMNLP 2025、KG + vector hybrid、cost 比 Microsoft 版低
- [**gusye1234/nano-graphrag**](https://github.com/gusye1234/nano-graphrag) — < 1000 行的最小实作、适合先读懂原理
...
- 已经在使用 Claude / 想用 prompt caching（其他 model 也能跑、就是没 cache 折扣）
...
**为什么是「必加 polish」**：production RAG 评测几乎一面倒——加 hybrid + reranker 后 recall@5 通常从 70% 上下提到 85-90%、边际成本低、实作成熟。**这是 cost / benefit 最好的两个改动**。
...
**代表工具**：
- **Hybrid search**：[Weaviate](https://github.com/weaviate/weaviate)（内建 BM25 + vector + RRF）/ [Qdrant](https://github.com/qdrant/qdrant)（支持 sparse + dense vector）/ pgvector + Postgres FTS
...
- **Framework 内建**：LlamaIndex 的 `SentenceTransformerRerank` / LangChain 的 `ContextualCompressionRetriever`
...
- [**RAG Fusion (Raudaschl 2023)**](https://github.com/Raudaschl/rag-fusion) — Multi-Query + RRF 的 reference 实作
- LangChain 内建 `MultiQueryRetriever` / LlamaIndex `HyDEQueryTransform`

### 🔁 Self-improving RAG — Self-RAG / CRAG / Adaptive RAG（2024 主轴）

**Mental model**：上面所有 RAG 技巧都假设「query 来 → retrieve → generate」是固定 pipeline。Self-improving RAG 把这个 pipeline 变成**有判断能力的 agent loop**——LLM 自己决定要不要 retrieve、判断 retrieve 品质、不够就再查或改 query。**这是 2024 RAG 研究的主轴**。
...
**实作**：LangGraph 有官方 [Self-RAG cookbook](https://langchain-ai.github.io/langgraph/tutorials/rag/langgraph_self_rag/) + [CRAG cookbook](https://langchain-ai.github.io/langgraph/tutorials/rag/langgraph_crag/) + [Adaptive RAG cookbook](https://langchain-ai.github.io/langgraph/tutorials/rag/langgraph_adaptive_rag/)、可直接套。

### 🌳 RAPTOR — 阶层式递归 retrieval（ICLR 2024）

**Mental model**：vanilla chunking 把文件切扁平 chunk——但**整本书的主旨不在任何单一 chunk 里**。RAPTOR 把 chunk 递归聚类 + 摘要、建一棵**多层树**：底层 = 原 chunk、中层 = 一群相关 chunk 的摘要、顶层 = 全文摘要。retrieve 时可选整棵树搜索、或选特定抽象层。
...
- [**parthsarthi03/raptor**](https://github.com/parthsarthi03/raptor) — 官方 reference 实作
- LlamaIndex 内建 `RAPTOR pack`
...
| 技巧 | 一句话 | 年份 / Paper |
|---|---|---|
| **Sentence-Window Retrieval** | embed 句子、retrieve 后回传 ± N 句 window | LlamaIndex 内建 |
...
| **HippoRAG 2** | 海马回启发、KG + PageRank、跨文件 multi-hop 联想 | [Gutiérrez et al. 2025](https://arxiv.org/abs/2502.14802) ⭐ **2025** |
...
| **MiA-RAG**（Mindscape-Aware）| 先建文件高层摘要 mindscape、用它引导 retrieval 跟回答 | [Turing Post 2026 12 types](https://www.turingpost.com/p/12ragtypes) ⭐ **2026** |
| **QuCo-RAG**（Quality-Controlled） | 用 pretraining 统计判断该不该 retrieve、罕见 entity 触发查、减 hallucination | 同上 ⭐ **2026** |
| **MegaRAG** | 多模态 KG、长文件抽 entity + relation + 视觉、建层级图 | 同上 ⭐ **2026** |
| **TV-RAG** | training-free 时间感知 RAG、长影片 + 字幕 + 视觉对齐 | 同上 ⭐ **2026** |
...
| **RAGPart / RAGMask** | 对 RAG corpus poisoning 攻击的轻量防御 | [Turing Post 2026](https://www.turingpost.com/p/12ragtypes) ⭐ **2026** |
...
| **进阶：Contextual Retrieval** | [Anthropic cookbook](https://platform.claude.com/cookbook/capabilities-contextual-embeddings-guide) | Claude 搭配 prompt caching 的 contextual chunking（**详见下方 §进阶 RAG 技巧**） |
| **进阶：knowledge graph 推理** | [LightRAG](https://github.com/HKUDS/LightRAG) / [Microsoft GraphRAG](https://github.com/microsoft/graphrag) | knowledge graph + RAG、entity-relation 推理（**详见下方 §进阶 RAG 技巧**） |
...
2. agent 要記事：加 **mem0**（最简单的 memory layer）
3. 开始 production-scale：换成 **Qdrant** 或 **pgvector**
4. 想升级到进阶 RAG：看下方 §进阶 RAG 技巧 三个 subsection
...
| **Vector DB**<br>（hybrid） | [Weaviate](https://github.com/weaviate/weaviate) | ⭐⭐⭐⭐ | production 部署 + schema 约束 | 内建模组（text2vec / generative / classification）、schema 驱动、内建 BM25 + vector hybrid。★ 16k+ |
...
| **教材合集** | [patchy631/ai-engineering-hub](https://github.com/patchy631/ai-engineering-hub) | ⭐⭐⭐⭐ | 想看「同概念在不同情境怎么实作」 | 主题式 LLM / RAG / agent tutorial 集、Jupyter notebook、跨多个 stage 都用得上。★ 34k+、MIT |
